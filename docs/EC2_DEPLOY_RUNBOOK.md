...
This is the ordered path from the AWS console to supervising the box from
your phone. It adds the AWS-side steps the other docs assume (launching
the instance, security group, Tailscale install). For depth on any step it
...
- [ ] Version footer / `GET /api/health` matches the deployed checkout.

## 8. First real project
...
4. Optionally tick **hold-dispatch while awaiting approval** (Settings,
   next to merge policy) for zero unsupervised spend while a decision
   waits on you.
...
| What | How |
|---|---|
| Update the box | Setup & Health → **Update Hoopedorc**, or `cd /opt/hoopedorc && npm run update` |
