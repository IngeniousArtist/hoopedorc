...
test("VW14 live Docker: filesystem/network boundary, cancellation, ownership and missing-auth refusal", { skip: !enabled, timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const { imageId, runtimeId } = await driver.inspectProfile(profile);
    ...
    const states: string[] = [];
    async function worker(readOnly = false) { const identity = workerIdentity("live-boundary", runtimeId); identities.push(identity); return { identity, execution: await driver.prepare({ identity, profile, imageId, cwd, directory: join(root, identity.id), readOnly, transition: (state) => states.push(state) }) }; }
