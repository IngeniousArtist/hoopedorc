...
test("VW07: review a pending-task revision, refuse intervening changes, then apply exactly once", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "2 added · 1 revised · 0 retained" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "2 added · 1 revised · 1 retained" })).toBeVisible();
  await page.getByRole("button", { name: "Apply reviewed changes", exact: true }).click();
  await page.getByRole("button", { name: "Confirm apply", exact: true }).click();
  ...
  expect((await (await page.request.get(`/api/projects/${project.id}/tasks`)).json() as ListTasksResponse).tasks).toHaveLength(4);
});
