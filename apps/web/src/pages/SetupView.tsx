...
export function SetupView({
  ...
  const [section, setSection] = useState<(typeof SETUP_SECTIONS)[number]["id"]>("overview");
  ...
  return (
    <div className="max-w-3xl space-y-5">
      ...
      <p className="text-sm text-neutral-400">
        Check this installation's connections, model availability, and update status.
        Live model tests and deployment updates run only when you request them.
      </p>
      <SectionTabs id="setup" label="Setup sections" sections={SETUP_SECTIONS} selected={section} onSelect={setSection} />
      {updateInProgress && section !== "updates" && (
        <button type="button" onClick={() => setSection("updates")} className="min-h-10 rounded border border-blue-800 bg-blue-950/30 px-3 py-2 text-sm text-blue-200">
          Update in progress — view status
        </button>
      )}
      ...
      <SectionPanel group="setup" id="overview" selected={section}>
        ...
        {error && <div role="alert" className="space-y-1 text-sm text-red-300">
          <p>Could not check connections: {error}</p>
          <p className="text-xs">{health ? "Showing the last available results. " : ""}Use Re-check to try again.</p>
        </div>}
          ...
          <>
            ...
            >
              {health.checks.length === 0
                ? "No connection checks were reported. Use Re-check to try again."
                : health.allOk
                  ? "Connection checks passed."
                  : "Some connections need attention. Follow the details below, then re-check."}
            </div>
                ...
                <div key={c.name} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-neutral-200">
                      <span>{c.name}</span>
                      <span className={`text-xs ${c.ok ? "text-green-400" : "text-amber-300"}`}>{c.ok ? "Ready" : "Needs attention"}</span>
                    </div>
      ...
      <SectionPanel group="setup" id="models" selected={section}>
        <p className="text-sm text-neutral-400">Review recorded reliability before using a live test. Opening this section does not send test prompts.</p>
        {/* Model health (F6) */}
        <div className="space-y-3 rounded-lg border border-neutral-800 bg-neutral-900 p-4">
          ...
          {healthLoading && !modelHealth && <p role="status" className="text-sm text-neutral-400">Loading model health…</p>}
          {modelHealthError && <div role="alert" className="space-y-1 text-sm text-red-300">
            <p>Could not load model health: {modelHealthError}</p>
            <p className="text-xs">{modelHealth ? "Showing the last available results. " : ""}Use Refresh to try again.</p>
          </div>}
          {!healthLoading && !modelHealthError && modelHealth?.models.length === 0 && (
            <p role="status" className="text-sm text-neutral-400">No configured models were reported. Add or enable models in Settings.</p>
          )}
        ...
        <div className="space-y-3 rounded-lg border border-neutral-800 bg-neutral-900 p-4">
          <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-[11px] text-neutral-400">
                Asks every enabled model to say hello and name itself. Costs
                money when your provider meters usage; subscription limits still apply.
              </p>
          ...
          {modelTestError && <p role="alert" className="break-words text-sm text-red-300">Could not test models: {modelTestError}</p>}
          {modelTest?.results.length === 0 && <p role="status" className="text-sm text-neutral-400">No enabled models were tested. Check your models in Settings.</p>}
          {modelTest && (
            <>
      ...
      <SectionPanel group="setup" id="updates" selected={section}>
        <p className="text-sm text-neutral-400">Manage this deployment. Availability and safety checks are enforced by the server.</p>
      ...
      <p className="flex flex-wrap gap-4 text-sm">
        <a href="#/settings" className="inline-flex min-h-10 items-center text-blue-400 underline underline-offset-4 focus-visible:ring-2 focus-visible:ring-blue-500">Open settings</a>
        <a href="https://github.com/IngeniousArtist/hoopedorc/blob/main/docs/USER_GUIDE.md" target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center text-neutral-400 underline underline-offset-4 focus-visible:ring-2 focus-visible:ring-blue-500">Installation guide ↗</a>
      </p>
    </div>
  );
}
