...
function TaskReview({ projectId, taskId, onAddToPlan }: { projectId: string; taskId: string; onAddToPlan: (text: string) => void }) {
  ...
  return <div className="space-y-4">
    ...
    {data && <>
      ...
      <nav className="flex flex-wrap gap-2" aria-label="Review sections">{(["Preview", "Changes", "Checks", "Activity"] as const).map((name) => <button key={name} className={`${control} ${tab === name ? "border-blue-500 text-blue-300" : ""}`} aria-pressed={tab === name} onClick={() => setTab(name)}>{name}</button>)}</nav>
      {tab === "Preview" && <>
      ...
      {tab === "Checks" && <section className={panel} aria-label="Code checks"><h3 className="font-medium">Required code checks</h3><p className="text-sm text-neutral-400">These are recorded validator decisions for their run. Legacy records do not include a commit hash, so they are historical evidence. Browser or design review cannot override them.</p>
        ...
        {data.decisions.map((decision) => <article key={decision.id} className="space-y-2 border-t border-neutral-800 pt-3"><p className="break-words text-sm">{decision.verdict.replaceAll("_", " ")} · {decision.validatorModel} · run {decision.runId}</p>
